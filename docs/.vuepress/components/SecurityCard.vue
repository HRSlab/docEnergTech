<template>
  ...
  >
    <div class="Subhead p-2">
      ...
      <p class="Subhead-description text-small">
        ...
        <span class="color-fg-accent">
          {{
            Math.abs(
              new Date(new Date($frontmatter.Security.latest) - Date.now())
                .getHours()
                .toLocaleString("it-IT", { hours: "numeric" })
            )
          }}
          ore fa
        </span>
    ...
    <ul class="Box Box--condensed color-shadow-medium anim-hover-grow">
      <li
        class="Box-row text-small"
        v-for="check in $frontmatter.Security.Checks"
        :key="check"
      >
        <svg
          class="octicon shield-check color-fg-success mr-2"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 16 16"
          width="16"
          height="16"
